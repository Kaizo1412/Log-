...
#Gridx1353
labels = ['Task Given:', 'Done:', 'Comment:']
for i in range(3):
	Label(root, text=labels[i]).grid(row=i, column=0, padx=10, pady=10)

BuList = Button(root, text='Log Report')
...
SpanDone = StringVar()
Radiobutton(root, text='YES', value='YES', variable=SpanDone).grid(row=1, column=1)
Radiobutton(root, text='NO', value='NO', variable=SpanDone).grid(row=1, column=2)
comment = Text(root, width=40, height=5, font=('Arial', 14))
comment.grid(row=2, column=1, columnspan=2, padx=10, pady=10)
#brought to you by code-projects.org
def SaveData():
	msg = conn.Add(fullname.get(), SpanDone.get(), comment.get(1.0, 'end'))
